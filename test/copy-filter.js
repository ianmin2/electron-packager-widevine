...
test('populateIgnoredPaths ignores the generated temporary directory only on Linux', t => {
    ...
    const expected = path.join(tmpdir, 'electron-packager-widevine')
    const opts = { tmpdir }
...
test('ignore: only match subfolder of app', util.testSinglePlatform(ignoreTest, 'electron-packager-widevine', path.join('electron-packager-widevine', 'readme.txt')))
